...
import { AiStage, type Slider, type TerraformConfig, TerraformTarget, ControlConfig, SoundConfig, Modulation, ModulationSource, ModulationTarget, CameraData, ViewMode, ShipConfig, ShipModulation, ShipModulationTarget, FlightMode } from '../types';
...
export const useAppStore = (): Omit<AppContextType, keyof ReturnType<typeof useDummyHandlers>> => {
  ...
  const [flightMode, setFlightMode] = useState<FlightMode>('N');
  const flightModeRef = useRef<FlightMode>('N');
  ...
  const analogInputRef = useRef({ left: { x: 0, y: 0 }, right: { x: 0, y: 0 } });
  ...
  useEffect(() => {
    ...
    const gameLoop = (timestamp: number) => {
        ...
        const analog = analogInputRef.current; // Read analog joystick values
        const currentMode = flightModeRef.current;

        // --- Physics & Camera (Drone Mode 2 Implementation) ---
        // Mode 2:
        // Left Stick Y (Up/Down) -> Ascend/Descend (Throttle) -> keys: Space/Shift
        // Left Stick X (Left/Right) -> Yaw (Rotate) -> keys: ArrowRight/ArrowLeft
        // Right Stick Y (Up/Down) -> Pitch (Forward/Back) -> keys: W/S
        // Right Stick X (Left/Right) -> Roll (Strafe) -> keys: D/A

        const keys = keysPressed.current;
        
        // Forward/Back (Pitch) - Right Stick Y
        let fwd = (keys.has('w') ? 1 : 0) - (keys.has('s') ? 1 : 0);
        fwd += analog.right.y; // Add analog input (-1 to 1)
        if (controls.invertForward) fwd = -fwd;

        // Strafe Left/Right (Roll) - Right Stick X
        let str = (keys.has('d') ? 1 : 0) - (keys.has('a') ? 1 : 0);
        str += analog.right.x; // Add analog input (-1 to 1)
        if (controls.invertStrafe) str = -str;

        // Ascend/Descend (Throttle) - Left Stick Y
        let asc = (keys.has(' ') ? 1 : 0) - (keys.has('shift') ? 1 : 0);
        // Changed to substract based on user request to invert logic
        asc -= analog.left.y; 
        if (controls.invertAscend) asc = -asc;
        
        // Camera Look Up/Down (Gimbal Pitch) - Controlled by keys or UI only, not sticks in Mode 2
        let pitchInput = (keys.has('arrowdown') ? 1 : 0) - (keys.has('arrowup') ? 1 : 0);
        if (controls.invertPitch) pitchInput = -pitchInput;

        // Yaw (Rotation) - Left Stick X
        // Standard: Right stick push -> Turn Right (positive rotation)
        // keys: Right Arrow (positive) - Left Arrow (negative)
        let yawInput = (keys.has('arrowright') ? 1 : 0) - (keys.has('arrowleft') ? 1 : 0);
        yawInput += analog.left.x; // Add analog input (-1 to 1)
        if (controls.invertYaw) yawInput = -yawInput;

        const [p, y] = cameraRef.current.rotation;
        const currentPos = cameraRef.current.position;
        
        // Flight Modes (Tripod/Normal/Sport) multipliers
        let spd = 1.0;
        let smoothFactor = 0.1;
        let rotSensitivity = 1.0;
        
        if (currentMode === 'C') { // Cine / Tripod
            spd = 0.4;
            smoothFactor = 0.05; // Very smooth
            rotSensitivity = 0.5;
        } else if (currentMode === 'S') { // Sport
            spd = 2.5;
            smoothFactor = 0.2; // Aggressive
            rotSensitivity = 1.5;
        } else { // Normal
            spd = 1.0;
            smoothFactor = 0.1;
        ...
        const rotSpd = 1.0 * rotSensitivity;
        ...
        cameraVelocityRef.current[0] += (tVX - cameraVelocityRef.current[0]) * smoothFactor;
        cameraVelocityRef.current[1] += (tVY - cameraVelocityRef.current[1]) * smoothFactor;
        cameraVelocityRef.current[2] += (tVZ - cameraVelocityRef.current[2]) * smoothFactor;
